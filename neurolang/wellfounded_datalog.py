from .expressions import (
    FunctionApplication, Constant, NonConstant, NeuroLangException,
...
class WellFoundedDatalog():
    ...
    Given a program P, the intuition behind the solution is the following:
        - The sequence starts with an overestimate of the negative facts in the
         answer, called I0 (it contains all negative facts).
        - From this overestimate, we compute I1 which includes all positive
        facts that can be infered from I0. This is an overestime of the
        positive facts in the answer, so the set of negative facts in I1 is an
        underestimate of the negative facts in the answer.
        - By continuing the process, we see that the even-indexed instance
        provide underestimates of the positive facts in th answer and the
        odd-indexed instances provide underestimates of the negative facts in
        the answer.
        - Finally, the 3-valued instance consisting of the facts known in both
        sequences becomes the well-founded semantics of P (all positive and
        negative facts belonging to all 3-stable model of P).
    '''
    ...
    def _deny_fact(self, expression: Expression) -> None:
        '''
        This function receives one expression that has to be negated and 
        includes it in the set of negated facts needed to form I0.
    ...
    def solve(self, eb: ExpressionBlock, datalog_program) -> tuple:
        '''
        Given an Expression Block and a Datalog Program, this function 
        calculate the Well-Founded Semantics using the WellFoundedEvaluator 
        and iterating between the overestime of the positive and negative facts
        until converge
        ...
        while (not_converged):
            ...
            for elem in evaluated:
                if (
                    hasattr(elem.functor, 'value') 
                    and elem.functor.value == invert
                ):
            ...
            not_converged = (
                self._converged_positive != self._overestimation_positive
                or self._converged_negative != self._overestimation_negative
            )

        result = ()
        for k, v in self._converged_negative.items():
...
class WellFoundedEvaluator(PatternWalker):
    ...
    def eval_function_application(self, expression):
        ...
        if f_name in self._new_symbols:
            if (
                hasattr(self._new_symbols[f_name].functor, 'value') 
                and self._new_symbols[f_name].functor.value == invert
            ):
                return 0
            else:
                return 1
