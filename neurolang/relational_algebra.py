...
class ExtendedProjection(RelationalAlgebraOperation):
    ...
    def __eq__(self, other):
        ...
        return (
            ...
            and len(self.projection_list) == len(other.projection_list)
            and all(
                any(
...
class StringArithmeticWalker(ew.PatternWalker):
    ...
    def constant_int(self, cst):
        return Constant[RelationalAlgebraStringExpression](
            str(cst.value), auto_infer_type=False, verify_type=False
        )
    ...
    def constant_str(self, cst):
        return Constant[RelationalAlgebraStringExpression](
            f'"{cst.value}"', auto_infer_type=False, verify_type=False
        )
...
class RelationalAlgebraSolver(ew.ExpressionWalker):
    ...
    @ew.add_match(Selection(..., FunctionApplication))
    ...
    def ra_rename_columns(self, rename_columns):
        ...
        new_set = relation.value
        renames = {src.value: dst.value for src, dst in rename_columns.renames}
    ...
    def _compile_function_application_to_sql_fun_exp(self, fun_exp):
        if isinstance(fun_exp, FunctionApplication) or isinstance(
            fun_exp, Constant[Column]
        ):
            try:
                ...
                return lambda t: fun(**{arg: getattr(t, arg) for arg in args})
        else:
    ...
    def set_destroy(self, destroy):
        ...
        if src_column not in relation.columns:
            raise NeuroLangException(
                f"source column {src_column} not present in " "set's columns"
            )
        ...
        for t in row_group_iterator:
            ...
            for row in t:
                row_set = set_type(
                    columns=dst_columns, iterable=getattr(row, src_column)
                )
                destroyed_set = destroyed_set | row_set
            new_set = t.projection(*cols).naturaljoin(destroyed_set)
...
class RelationalAlgebraPushInSelections(ew.PatternWalker):
    ...
    @ew.add_match(
        ...
        lambda exp: len(
            set(exp.relation.attributes) & get_expression_columns(exp.formula)
        )
        == 0,
    )
