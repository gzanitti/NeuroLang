...
def test_conjunctive_expression():
    ...
    assert is_conjunctive_expression(Q())

    assert is_conjunctive_expression(Q(x))

    assert is_conjunctive_expression(DT.walk(Q(x) & R(y, C_(1))))

    assert not is_conjunctive_expression(DT.walk(R(x) | R(y)))

    assert not is_conjunctive_expression(DT.walk(R(x) & R(y) | R(x)))

    assert not is_conjunctive_expression(DT.walk(~R(x)))

    assert not is_conjunctive_expression(R(Q(x)))
...
def test_conjunctive_expression_with_nested():
    ...
    assert is_conjunctive_expression_with_nested_predicates(Q())

    assert is_conjunctive_expression_with_nested_predicates(Q(x))

    assert is_conjunctive_expression_with_nested_predicates(Q(x) & R(y, C_(1)))

    assert is_conjunctive_expression_with_nested_predicates(
        Q(x) & R(Q(y), C_(1))
    )

    assert not is_conjunctive_expression_with_nested_predicates(R(x) | R(y))

    assert not is_conjunctive_expression_with_nested_predicates(
        R(x) & R(y) | R(x)
    )

    assert not is_conjunctive_expression_with_nested_predicates(~R(x))
...
def test_extract_datalog_predicates():
    ...
    expression = DT.walk(B_([Q(x), Q(y) & ~R(y)]))
    assert extract_logic_predicates(expression) == {Q(x), Q(y), Negation(R(y))}
...
class Datalog(DatalogProgram, ExpressionBasicEvaluator):
    pass
...
def test_dependency_matrix():
    ...
    assert np.array_equiv(
        dep_matrix, np.array([[0, 0, 1], [1, 1, 0], [0, 0, 0]])
    )
    ...
    idb_symbols_2, dep_matrix_2 = dependency_matrix(datalog, rules=rules)
