...
class TranslateToNamedRA(ExpressionBasicEvaluator):
    ...
    def translate_conjunction(self, expression):
        ...
        while (
            ...
            new_output = TranslateToNamedRA.process_extended_projection_formulas(
                classified_formulas, new_output
            )
    ...
    def process_equality_formula(formula, named_columns, output):
        ...
        if left in named_columns and right in named_columns:
            ...
            output = Selection(
                NaturalJoin(output, RenameColumn(output, left, right)),
                criteria,
            )
            named_columns.add(right)
        elif right in named_columns:
            output = Selection(
                NaturalJoin(output, RenameColumn(output, right, left)),
                criteria,
            )
