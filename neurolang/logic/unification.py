...
from . import NaryLogicOperator, UnaryLogicOperator
...
def most_general_unifier(expression1, expression2):
    ...
    if unifier is None:
        ...
        return (unifier[0], apply_substitution(expression1, unifier[0]))
...
def most_general_unifier_extract_arguments(expression1, expression2):
    ...
    while expression_stack:
        ...
        if not (
            expression1.functor == expression2.functor
            and len(expression1.args) == len(expression2.args)
        ):
            return None, None
...
def most_general_unifier_arguments(args1, args2):
    ...
    If the MGU doesn't exist it returns None.
    """
    if not (isinstance(args1, tuple) and isinstance(args2, tuple)):
